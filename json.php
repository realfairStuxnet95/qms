		...
		if(count($response)>0){
			...
			if($approve){
				...
				foreach ($response as $key => $value) {
					$data[]=array("trainee_names"=>$value['trainee_names'],"trainee_number"=>$value['trainee_number'],"reg_number"=>$value['training_id'],"train_time"=>$value['train_time'],"pc"=>$upload->getTable($pc_id));
				}
